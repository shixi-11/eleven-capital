...
function servicesSection(c) { return `<section class="section focus" id="focus" aria-labelledby="focus-title"><div class="section-heading"><p class="section-label">${c.focusLabel}</p><div><h2 id="focus-title">${lines(c.focusTitle)}</h2><p class="section-intro">${c.focusIntro}</p></div></div>${serviceGallery(c)}<div class="service-list">${c.services.map((s, i) => `<article class="service" id="${s.id}"><span class="service-number" aria-hidden="true">0${i + 1}</span><div><h3>${s.title}</h3>${s.en ? `<p class="service-en" lang="en">${s.en}</p>` : ""}<p>${s.text}</p><div class="service-delivery"><h4>${c.deliveryLabel}</h4><p>${c.deliveries[i]}</p></div><ul class="service-examples">${s.examples.map(example => `<li>${esc(example)}</li>`).join("")}</ul></div></article>`).join("")}</div><section class="engagement" aria-labelledby="engagement-title"><p class="section-label">${c.engagementLabel}</p><h3 id="engagement-title">${c.engagementTitle}</h3><p class="engagement-intro">${c.engagementIntro}</p><div class="engagement-steps">${c.engagementSteps.map(([title, text]) => `<div><h4>${title}</h4><p>${text}</p></div>`).join("")}</div></section>${faqSection(c)}</section>
`; }
